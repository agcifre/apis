		...
		<soapenv:Envelope>
			<soapenv:Body>
				<han:getFileByDateResponse>
					<xsl:attribute name="customerTicket"><xsl:value-of select="//*[local-name()='ConsultFilesBydateResult']/@*[local-name()='customerTicket']"/></xsl:attribute>
					<xsl:attribute name="requestId"><xsl:value-of select="//*[local-name()='ConsultFilesBydateResult']/@*[local-name()='requestId']"/></xsl:attribute>
					<xsl:choose>
						...
						<xsl:when test="$status/*[local-name()='value']">
							...
							<xsl:if test="(($status/*[local-name()='value']!='NO_RESULT' and $status/*[local-name()='value']!='NO_DATA_FOUND') or $status/*[local-name()='value']='INCOMPLETE_DATA')">
								<xsl:element name="han:fileByDate">
									<xsl:copy-of select="$fileByDate/*" copy-namespaces="no"/>
								</xsl:element>
							</xsl:if>
						</xsl:when>
						<xsl:otherwise>
